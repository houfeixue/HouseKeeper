...
-(void)conFigCellwithData:(id)data atIndex:(NSIndexPath *)indexPath
{
    if ([data isKindOfClass:[LKPictureModel class]]) {
        ...
        NSString * imagePath = [NSString stringWithFormat:@"%@/%@",LKShrinkPicturePath,model.picName];
        ...
        NSString * imagePath = [NSString stringWithFormat:@"%@%@%@",LKIconHost,model.url,LKNetShrinkPic_Default];

        [_picImageView yy_setImageWithURL:[NSURL URLWithString:imagePath] placeholder:[UIImage imageNamed:LKPicture_Default]];
        ...
        [_picImageView yy_setImageWithURL:[NSURL URLWithString:imagePath] placeholder:[UIImage imageNamed:LKPicture_Default]];
    }else if ([data isKindOfClass:[LKLookPicModel class]]){
        LKLookPicModel *model = (LKLookPicModel *)data;
        NSString * imagePath = [NSString stringWithFormat:@"%@%@%@",LKIconHost,model.url,LKNetShrinkPic_Default];
        [_picImageView yy_setImageWithURL:[NSURL URLWithString:imagePath] placeholder:[UIImage imageNamed:LKPicture_Default]];
    }
}
