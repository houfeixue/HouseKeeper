...
-(void)setChat:(SDChatDetail *)chat{
    ...
    if (chat.chatType==SDChatDetailTypeSystemMessages) //消息类型位系统消息
    {
        ...
        NSString *systemMessage =[NSString stringWithFormat:@"%@ %@",chat.timeStr,contentString];
    ...
    }else  //其他消息
    {
        ...
        CGSize timeStrSize= [chat.timeStr sizeWithFont:sdTimeFont maxSize:CGSizeMake(kScreen_Width, 20)];
        timeX = (kScreen_Width - timeStrSize.width-10)/2;

        self.timeFrame =CGRectMake(timeX, timeY, timeStrSize.width+10, timeH);
        ...
        CGFloat contentX ;
        CGFloat contentY =iconHeadY+nameH+5;
        ...
        switch (chat.chatType) {
            case SDChatDetailTypeText:
            {
                ...
                NSString *contentString = [NSString decodeEmoj:chat.chatMsg.message];
                CGSize contentStrSize =[contentString boundingRectWithSize:
