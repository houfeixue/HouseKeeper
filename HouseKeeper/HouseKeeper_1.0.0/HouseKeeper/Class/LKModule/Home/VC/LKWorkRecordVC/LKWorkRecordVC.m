...
@property (nonatomic, strong) LKWorkRecordPlaceholderView *noRecordPlaceholderView;
@end
...
- (void)createUI {
    ...
    self.noRecordPlaceholderView.backgroundColor = self.view.backgroundColor;
    [self.view addSubview:self.noRecordPlaceholderView];
    [self.view bringSubviewToFront:self.noRecordPlaceholderView];


    [self.noRecordPlaceholderView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.view);
    }];
    self.noRecordPlaceholderView.hidden = YES;
    ...
    self.noRecordPlaceholderView.addWorkRecord = ^{
        @strongify(self);
        LKAddWorkRecordVC *vc = [[LKAddWorkRecordVC alloc] init];
        vc.workDate = [self.calendarView.dateFormatter stringFromDate:self.currentSelectDate];
        vc.delegate = self;
...
- (void)bindViewModelData {
    ...
    [self.recordViewModel.loadDataSubject subscribeNext:^(id  _Nullable x) {
        @strongify(self);
        BOOL isHasData = self.recordViewModel.dataArray.count > 0 ? YES : NO;
        self.noRecordPlaceholderView.hidden = isHasData == NO ? NO : YES;
...
- (LKWorkRecordPlaceholderView *)noRecordPlaceholderView {
    if (_noRecordPlaceholderView == nil) {
        _noRecordPlaceholderView = [[LKWorkRecordPlaceholderView alloc] init];
    }
    return _noRecordPlaceholderView;
}
@end
