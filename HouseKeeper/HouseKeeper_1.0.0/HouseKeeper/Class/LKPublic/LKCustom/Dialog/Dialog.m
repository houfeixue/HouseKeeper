...
/** 一键评分 alert */
+ (void)showScoreAlertViewWithMessage:(NSString *)message placeholderString:(NSString *)placeholderString score:(CGFloat)score dismissHandler:(void (^)(NSString *, NSInteger, LKInputAlertView *))dismissHandler {
    [[UIApplication sharedApplication].keyWindow endEditing:YES];
...
+ (void)showScoreAlertViewWithMessage:(NSString *)message inputText:(NSString *)inputText placeholderString:(NSString *)placeholderString score:(CGFloat)score dismissHandler:(void (^)(NSString *, NSInteger, LKInputAlertView *))dismissHandler {
    [[UIApplication sharedApplication].keyWindow endEditing:YES];
    LKCustomAlertViewOption *firstOption = [[LKCustomAlertViewOption alloc] initWithTitle:@"取消" color:[UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1.0]];
    LKCustomAlertViewOption *secondOption = [[LKCustomAlertViewOption alloc] initWithTitle:@"保存" color:[UIColor colorWithRed:82/255.0 green:113/255.0 blue:141/255.0 alpha:1.0]];
    LKInputAlertView *inputView = [[LKInputAlertView alloc] initWithTitle:@"一键评分" tipMessage:@"输入真实评分：" inputText:inputText placeholderString:placeholderString score:score message:message options:[NSMutableArray arrayWithArray:@[firstOption,secondOption]] selectCallBack:dismissHandler];
    [inputView showAlertView];
}

+ (void)showInputAlertViewWithTitle:(NSString *)title message:(NSString *)message tipMessage:(NSString *)tipMessage placeholderString:(NSString *)placeholderString score:(CGFloat)score firstButtonName:(NSString *)firstButtonName secondButtonName:(NSString *)secondButtonName dismissHandler:(void (^)( NSString *,NSInteger, LKInputAlertView *))dismissHandler {
