...
- (void)setupToolbars
{
    ...
    indexLabel.bounds = CGRectMake(0, 0, 200, 30);
...
- (void)saveImage
{
    ...
    UIImageView *currentImageView = scrollView.subviews[index];
...
{
    SDBrowserImageView *imageView = scrollView.subviews[index];
    ...
    if ([self highQualityImageURLForIndex:index]) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [imageView yy_setImageWithURL:[self highQualityImageURLForIndex:index] placeholder:[self placeholderImageForIndex:index]];
        });
    } else {
        imageView.image = [self placeholderImageForIndex:index];
    }
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:UICollectionView.class]) {
        ...
        if (self.sourceImagesContainerViewIndexPath != nil) {
            path = [NSIndexPath indexPathForItem:self.sourceImagesContainerViewIndexPath.item inSection:self.sourceImagesContainerViewIndexPath.section];
        }
...
- (void)layoutSubviews
{
    ...
    CGFloat y = 0;
    CGFloat w = scrollView.frame.size.width - SDPhotoBrowserImageViewMargin * 2;
    CGFloat h = scrollView.frame.size.height;
    
    
    
    [scrollView.subviews enumerateObjectsUsingBlock:^(SDBrowserImageView *obj, NSUInteger idx, BOOL *stop) {
        CGFloat x = SDPhotoBrowserImageViewMargin + idx * (SDPhotoBrowserImageViewMargin * 2 + w);
        obj.frame = CGRectMake(x, y, w, h);
    }];
    
    scrollView.contentSize = CGSizeMake(scrollView.subviews.count * scrollView.frame.size.width, 0);
    scrollView.contentOffset = CGPointMake(self.currentImageIndex * scrollView.frame.size.width, 0);
    ...
    _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, kStatusBarHeight + 15);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(UIView *)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"frame"]) {
        ...
        SDBrowserImageView *currentImageView = scrollView.subviews[_currentImageIndex];
...
- (void)showFirstImage
{
    ...
    CGRect targetTemp = [scrollView.subviews[self.currentImageIndex] bounds];
    
    tempView.frame = rect;
    tempView.contentMode = [scrollView.subviews[self.currentImageIndex] contentMode];
    scrollView.hidden = YES;
