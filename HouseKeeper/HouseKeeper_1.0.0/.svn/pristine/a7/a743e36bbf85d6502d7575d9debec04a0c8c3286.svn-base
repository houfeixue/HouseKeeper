...
- (void)setSelectImageModel:(LKPictureModel *)selectImageModel {
    ...
    if ( _selectImageModel == nil || [LKCustomTool isBlankString:_selectImageModel.picName]) {
        ...
        if ([_selectImageModel.picName hasPrefix:LKCompoundPicturePath] ) {
            [self.imageView yy_setImageWithURL:[NSURL fileURLWithPath:_selectImageModel.picName] placeholder:[UIImage imageNamed:LKPicture_Default]];
        }else {
            [self.imageView yy_setImageWithURL:[[NSString stringWithFormat:@"%@%@",LKIconHost,_selectImageModel.picName] toURL] placeholder:[UIImage imageNamed:LKPicture_Default]];
        }
    }
