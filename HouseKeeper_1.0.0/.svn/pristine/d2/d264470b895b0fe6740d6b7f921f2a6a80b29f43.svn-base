...
- (void)bindViewModelData {
    ...
    [self.communityResultViewModel.loadCommunityDataSubject subscribeNext:^(id  _Nullable x) {
        ...
        [self.communityResultViewModel changeSelectDataArray:[self.selectCommunityArray mutableCopy]];
    ...
    [[[[[self.searchBar.searchTextField rac_textSignal] throttle:0.2f] distinctUntilChanged] ignore:nil] subscribeNext:^(NSString * _Nullable x) {
        @strongify(self);
        if (self.searchBar.searchTextField.markedTextRange == nil) {
...
- (void)createBottomView {
    ...
    self.bottomView.checkSelectCommunity = ^{
        ...
        [vc.checkSureSelectSubject subscribeNext:^(NSMutableArray * _Nullable checkSelectCommunityArray) {
            @strongify(self);
            [self.communityResultViewModel changeSelectDataArray:[checkSelectCommunityArray mutableCopy]];
