...
//    }];
    ...
    [self.timeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.extraView.mas_right).inset(LKLeftMargin);
    ...
    [self.tipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(LKLeftMargin);
        make.right.equalTo(self.extraView.mas_right).inset(LKLeftMargin);
        make.bottom.equalTo(self.extraView.mas_bottom).inset(LKLeftMargin);
    }];
...
- (UIView *)extraView {
    if (_extraView == nil) {
        ...
        _extraView.backgroundColor = RGBA(0, 0, 0, 0.4f);
    }
    return _extraView;
}
@end
