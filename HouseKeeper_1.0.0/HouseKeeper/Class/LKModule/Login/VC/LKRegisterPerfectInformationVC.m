...
- (void)bindActionHandle {
    ...
    [[self.addressItemView.nextBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        vc.selectCommunityArray = [self.selectCommunityModelArray mutableCopy];
        [self.navigationController pushViewController:vc animated:YES];
        @weakify(self);
        [vc.sureSelectSubject subscribeNext:^(NSMutableArray *  _Nullable selectCommunityArray) {
            @strongify(self);
            self.selectCommunityModelArray = [selectCommunityArray mutableCopy];
            [self changeSelectCommunityTextField];
        }];
    }];
    /** 选择管家身份 */
...
- (void)changeSelectCommunityTextField {
    if (self.selectCommunityModelArray.count > 0) {
        ...
        if (self.selectCommunityModelArray.count == 1) {
            ...
            self.addressItemView.textField.text = [NSString stringWithFormat:@"%@等%lu个小区",communityModel.name,(unsigned long)self.selectCommunityModelArray.count];
        }
    }else {
...
- (void)createItemView {
    ...
    LKUserInfoModel *userInfoModel = [LKCustomMethods readUserInfo];
    NSString *name = @"";
    if (userInfoModel != nil && [LKCustomTool isBlankString:userInfoModel.name] == NO) {
        name = userInfoModel.name;
    }
    [self.nameItemView bindDataWithIconImage:@"app_icon_name" placeholder:@"管家姓名" textFieldText:name textFieldCanEdit:YES isShowNextBtn:NO];
    ...
    @weakify(self);
    [[self.idCardItemView.textField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        @strongify(self);
        if (x.length >= 18) {
            x = [x substringToIndex:18];
            self.idCardItemView.textField.text = x;
        }
    }];
...
- (LKRequestViewModel *)createDataSource {
    return  self.perfectInformationViewModel;
}
@end
