...
-(void)conFigUI{
    ...
    [_standardLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.equalTo(@(kScreen_Width-80-15));
    ...
    NSString *scoreString = [NSString stringWithFormat:@"%lf",self.selectModel.scort];
    if (![self.selectModel.endFlag isEqualToNumber:@(0)]) {
        NSDecimalNumber *decNumber = [NSDecimalNumber decimalNumberWithString:scoreString];
        _checkScoreView.textField.text = [decNumber stringValue];
    }else{
        _checkScoreView.textField.text = @"";
...
- (void)keyboardWillShow:(NSNotification *)notification {
    if (self.scoreDetailView.textView.isFirstResponder == YES) {
        self.systemKeyboardHeight = [[notification.userInfo objectForKey:@"UIKeyboardBoundsUserInfoKey"] CGRectValue].size.height + 44 ;
        [self.scoreDetailView layoutIfNeeded];
        CGRect frame = [[UIApplication sharedApplication].keyWindow convertRect:self.scoreDetailView.frame fromView:self.mainScroll];
        if (CGRectGetMaxY(frame) + self.systemKeyboardHeight  > kScreen_Height ) {
            [self.mainScroll setContentOffset:CGPointMake(0, self.scoreDetailView.height - 50)];
        }
    }
}
...
- (void)LKAddWorkDescriptionItemViewDelegateViewHeight:(CGFloat)viewHeight {
    ...
    CGRect frame = [[UIApplication sharedApplication].keyWindow convertRect:self.scoreDetailView.frame fromView:self.mainScroll];
    if (CGRectGetMaxY(frame) + self.systemKeyboardHeight > kScreen_Height ) {
        [self.mainScroll setContentOffset:CGPointMake(0, viewHeight - 50)];
    }
}
